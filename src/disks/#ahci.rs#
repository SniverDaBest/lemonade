...
impl fmt::Display for AHCIController {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "PCI Device: {} | Base Address: {}", self.pci_device, self.base_addr
        )
    }
}
...
pub fn scan_for_used_ports(controller: &AHCIController, verbose: bool) -> Vec<AHCIDevice> {
    ...
    for port in 0..num_ports {
        ...
        if device_detected {
            if verbose { println!("Device detected on port {}", port); }
            devices.push(AHCIDevice::new(port,controller));
        } else {
            if verbose { println!("No device detected on port {}", port); }
        }
    }
